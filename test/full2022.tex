%LaTeX convention card editor

%v1.0 - Gordon Bower - 17 December 2017
%v1.0.1 - alignment fixes, default text formatting in style file
%v2.0 - Michael Farebrother - convert to 2022 card - Feb 24 2022

%Fill out the fields below to create your own ACBL Convention Card in LaTeX
%grbcce2022.sty contains the TikZ formatting instructions

%grbcce requires the following packages: tikz, txfonts, microtype, ifthen, xstring

\documentclass[12pt]{article}
%the card is 8'' wide, so the margins are very narrow to fit on letter size paper:
\usepackage{template/grbcce2022}
\usepackage[left=0mm,right=5mm,top=20mm]{geometry}
\begin{document}

%There is a boolean to turn on or off every checkbox on the card.
%To fill in any checkbox, remove the ``%'' in front of the \setboolean statement
%that activates that particular checkbox.
%You may delete unused setboolean statements.

%There is a text item to insert a description on every blank line of the card.
%You WILL get errors if you delete any \newcommand statements!
%if you want to leave a line blank, use \newcommand{\nameoftextfield}{} .

%If you insert plain text into any text field, default formatting is applied.
%To override my default, use any of the following predefined styles:
%\regtext{...} for regular black text
%\redreg{...} for regular red text (for conventions you will alert)
%\bluereg{...} for regular blue text (for announcements)
%\boldtext{...} for bold black text
%\redbold{...} for bold red text
%\bluebold{...} for bold blue text (e.g. more visible 1NT ranges)
%Use whatever other LaTeX formatting you wish. Default font size is \scriptsize.

%To insert (black) suit symbols: \s, \h, \d, \c.
%To insert a red suit: \rd, \rh
%To insert a red spade: \textcolor{red}{\s}

%To specify what card you lead from a given holding, indicate that card's
%POSITION in the given sequence:
%e.g. if you lead T from KJTx, \newcommand{\KTJx}{3}
%You MAY circle more than one card if you wish. e.g., \newcommand{\TNxx}{14}
%If you use the default, you may choose to circle none, \newcommand{\TNxx}{}

%You WILL get errors if you delete any \newcommand statements!
%If you lead the default card, you may use \newcommand{cardcombination}{}.

\newcommand{\names}{Eric Hollingsworth -- Duck Dunn\namefix{}}
\newcommand{\playernumber}{1234567/Quack}
\setboolean{visib}{true}  % show guidelines
% Select Exactly One of the following three lines for "user input serif?"
% top is "serif only if no guidelines" (otherwise output can be a bit confusing)
% but you can force yes or no with the next lines.
\ifthenelse{\boolean{visib}}{}{\setboolean{serif}{true}}
%\setboolean{serif}{true}
%\setboolean{serif}{false}

%General approach:
\newcommand{\generalapproach}{Sub-standard Amerjcan\namefix{}}
\newcommand{\minHCPopen}{11++}
\newcommand{\minHCPresp}{Ace or 6}
\setboolean{fo1c}{true} %Forcing openings...
\setboolean{fo2c}{true}
\newcommand{\foothertext}{Hello}%Other forcing opening bids
\setboolean{1nts}{true} % 1NT strength
\setboolean{1ntw}{true}
\setboolean{1ntv}{true}
\newcommand{\bidspreptop}{Mathé/Strong Club\pfix{}} % Bids that Require Preparation
\newcommand{\bidsprepbottom}{Keri/NT, Multi 2\rd\ } % Bids that Require Prep line 2

% 1 Club
\setboolean{club5}{true} %1C length
\setboolean{club4}{true}
\setboolean{club3}{true}
\setboolean{cl2nf}{true}
\setboolean{c4432}{true}
\setboolean{cl1nf}{true}
\setboolean{cl0nf}{true}
\setboolean{clcon}{true}
\newcommand{\clresponses}{1M, 2m GF 5+}
\setboolean{ctxfr}{true} % transfer responses to 1C
\newcommand{\cldescribe}{weaker minor if BAL} % Insert text in blank space next to 1C

\newcommand{\clrespdi}{0--7} % text after "1D response"
\setboolean{bypas}{true}
\newcommand{\minonenlowtext}{6} %1C-1NT range
\newcommand{\minonenhightext}{10-}
\newcommand{\mintwonlowtext}{10+}
\newcommand{\mintwonhightext}{12}

\setboolean{clsrn}{true} % Single raise - NF
\setboolean{clsri}{true} %              - Inv+
\setboolean{clsrf}{true} %              - GF
\setboolean{cljrw}{true} % jump raise - Weak
\setboolean{cljrm}{true} %            - Mixed
\setboolean{cljri}{true} %            - Inv
\setboolean{clocw}{true} % After overcall - NF
\setboolean{clocm}{true} %                - Mixed
\setboolean{cloci}{true} %                - Inv

% 1 Diamond
\setboolean{diam5}{true} %1D length
\setboolean{diam4}{true}
\setboolean{diam3}{true}
\setboolean{dubal}{true}
\setboolean{di2nf}{true}
\setboolean{di1nf}{true}
\setboolean{di0nf}{true}
\setboolean{dicon}{true}
\newcommand{\diresponses}{1M = other major\pfix{}}
\setboolean{dascl}{true} % 1D responses like 1C
\newcommand{\didescribe}{4 unless 4-4-3-2} % Insert text in blank space next to 1D e.g. ``4 unless 4-4-3-2''

\newcommand{\minonedionenlowtext}{6} %1D-1NT range
\newcommand{\minonedionenhightext}{10-}
\newcommand{\minoneditwonlowtext}{10+}
\newcommand{\minoneditwonhightext}{12}

\setboolean{disrn}{true} % Single raise - NF
\setboolean{disri}{true} %              - Inv+
\setboolean{disrf}{true} %              - GF
\setboolean{dijrw}{true} % jump raise - Weak
\setboolean{dijrm}{true} %            - Mixed
\setboolean{dijri}{true} %            - Inv
\setboolean{diocw}{true} % After overcall - NF
\setboolean{diocm}{true} %                - Mixed
\setboolean{dioci}{true} %                - Inv

%Major openings:
\setboolean{h12l4}{true} %4 cards in 1st/2nd seat
\setboolean{h12l5}{true}
\setboolean{h34l4}{true} %4 cards in 3rd/4th seat
\setboolean{h34l5}{true}
\setboolean{ntfor}{true} %1NT forcing
\setboolean{ntsem}{true} %1NT semiforcing
\setboolean{ntbps}{true} %1H-1NT bypass 4+S
\newcommand{\majothertexttop}{HSGT, fit J/S in comp\pfix{}} %Other major methods: HSGT, Mathe asking bid, etc
\newcommand{\majothertextbottom}{1\rh -- 2\s\, J2N} %Other major methods line two

\setboolean{ra2nt}{true} %Jacoby
\setboolean{ra3nt}{true} %1M-3NT
\setboolean{raspl}{true} %Splinter
\newcommand{\majraisetext}{J/S mini Splinter, Kokish GTry\pfix{}} %Text line for Bergen, fitjumps, etc
\setboolean{dru2c}{true} %What kind of Drury?
\setboolean{dru2d}{true}
\setboolean{drcmp}{true}
\newcommand{\majdrurytext}{over X}
\setboolean{dblrw}{true} %double raise
\setboolean{dblrm}{true}
\setboolean{dblri}{true}
\setboolean{ocdrw}{true} %double raise in comp
\setboolean{ocdrm}{true}
\setboolean{ocdri}{true}

%Notrump openings:
%First column:
\newcommand{\ntlowtext}{15} %NT range
\newcommand{\nthightext}{17}
\newcommand{\ntseatvul}{4th or Vul}
\newcommand{\altntlowtext}{12} %If variable NT, second range here
\newcommand{\altnthightext}{14}
\setboolean{ntasr}{true} %Same responses for alt NT range?
\newcommand{\altntresponse}{when we forget\pfix{}}
\setboolean{nt5cm}{true}
\newcommand{\sysontext}{X, 2\,\c} %Systems on after 1NT-(X), 2\c, etc...
\setboolean{staym}{true}
\setboolean{puppe}{true}
\setboolean{2coth}{true}
\setboolean{nat2d}{true}
\setboolean{jac2d}{true}
\newcommand{\ntditext}{GF Stay\pfix{}} %Other meaning for 1NT-2\d
\setboolean{nat2h}{true}
\setboolean{jac2h}{true}
\newcommand{\nthetext}{\s + m\xfix{}} %Other meaning for 1NT-2\h
\setboolean{nat2s}{true}
\setboolean{trf2s}{true}
\newcommand{\ntsptext}{\bluereg{Transfer to\,\bc}} %1NT-2S
\setboolean{nat2n}{true}
\setboolean{trf2n}{true}
\newcommand{\ntnttext}{\bluereg{Transfer to\,\rd}} %1NT-2NT

%NT openings - 2nd column:
\newcommand{\ntcjumptext}{minors INV} %1NT-3C
\newcommand{\ntdjumptext}{minors GF}
\newcommand{\nthjumptext}{Majors INV\pfix{}}
\newcommand{\ntsjumptext}{Majors GF\pfix{}} %1NT-3S
\newcommand{\ntothertexttop}{Over weak: 2-way, no smol/tex\pfix{}} %Other conventions over 1NT
\newcommand{\ntothertextbottom}{\hspace{4mm}and negX throughout\pfix{}} %blank line after texas transfers

% NT openings - bottom
\setboolean{smole}{true}
\setboolean{tex4c}{true}
\setboolean{tex4d}{true}
\setboolean{tex4h}{true}
\setboolean{ntneg}{true}
\newcommand{\ntnegtext}{3 level only\pfix{}} %Negative doubles over 1NT?
\setboolean{ntpen}{true}
\newcommand{\ntxothertext}{stolen bid} %Other doubles over 1NT
\setboolean{leben}{true}
\newcommand{\ntlebentext}{slow shows} %Lebensohl: Fast or slow denies?

%2NT/3NT openings
\newcommand{\twonlowtext}{20}
\newcommand{\twonhightext}{21}
\setboolean{pup2n}{true}
\newcommand{\twonsptext}{one- or two-minor slam try\pfix{}} %2NT-3S
\setboolean{con2n}{true}
\newcommand{\convnttext}{5--5 minors} % Conventional 2NT
\setboolean{jac2n}{true}
\setboolean{tex2n}{true}
\setboolean{neg2n}{true}
\newcommand{\twonothertext}{slam try\pfix{}} %Blank line in 2NT box

\newcommand{\threenlowtext}{24} %3NT range
\newcommand{\threenhightext}{27}
\setboolean{gam3n}{true}
\newcommand{\threenonesuittext}{solid suit (0 outside)\pfix{}} %One suit description 3NT box

%2C
\newcommand{\twoclowtext}{22+} %2C point range
\newcommand{\twochightext}{37}
\newcommand{\twocldescribe}{or 8.5 QT w/def} %line of text on left side of 2C divider
\setboolean{vs2cl}{true} %2C Very strong?
\setboolean{st2cl}{true}
\setboolean{na2cl}{true} %2C Natural?
\setboolean{ot2cl}{true} %2C conventional?
\newcommand{\twoclconvtext}{random hands}
\setboolean{neg2d}{true} %2D neg or waiting?
\setboolean{wai2d}{true}
\setboolean{steps}{true} %step/control responses?
\newcommand{\twoclstepstext}{01,2,AK,KKK,4+}
\setboolean{neg2h}{true}
\newcommand{\twoclresponse}{3NT psychic} %Other text on right side of 2C divider

% 2D
\newcommand{\twodlowtext}{5} %2D point range
\newcommand{\twodhightext}{11}
\newcommand{\twoddescribe}{disciplined 2nd and vul}
\setboolean{dnsnf}{true} %new suit NF
\setboolean{wk2di}{true} %2D weak
\setboolean{in2di}{true}
\setboolean{st2di}{true}
\setboolean{co2di}{true} %2D conventional
\newcommand{\twodntresp}{Ogust\pfix{}}
\newcommand{\twodresponse}{2M fr.psych\pfix{}}

%2H:
\newcommand{\twohlowtext}{5}
\newcommand{\twohhightext}{10}
\newcommand{\twohdescribe}{crazy, usually\pfix{}}
\setboolean{hnsnf}{true}
\setboolean{wk2he}{true}
\setboolean{in2he}{true}
\setboolean{st2he}{true}
\setboolean{2s2he}{true}
\newcommand{\twohntresp}{Ogust\pfix{}}
\newcommand{\twohresponse}{2\,\s\ fr.psych\pfix{}}
%2S:
\newcommand{\twoslowtext}{5}
\newcommand{\twoshightext}{10}
\newcommand{\twosdescribe}{Sometimes hearts}
\setboolean{snsnf}{true}
\setboolean{wk2sp}{true}
\setboolean{in2sp}{true}
\setboolean{st2sp}{true}
\setboolean{2s2sp}{true}
\newcommand{\twosntresp}{Ogust\pfix{}}
\newcommand{\twosresponse}{Bingo\pfix{}}

%Bottom of card:
\newcommand{\jshifttext}{Weak NC, Fit in comp or by PH\pfix{}} % jump shift responses
\newcommand{\vsstrongart}{Mathé} % "vs str/VS openings" - MDF to fix
\setboolean{nmf1w}{true} %1-way new minor
\setboolean{nmf2w}{true} %2-way new minor
\setboolean{xyz2w}{true} %2-way new minor
\setboolean{fsfrd}{true} %Fourth suit
\setboolean{fsfgf}{true}
\newcommand{\footnotestop}{Random information here} % 2 lines for additional conventions at bottom
\newcommand{\footnotesbottom}{and 1C responses here\pfix{}}

%Back of card:
\setboolean{negax}{true} %Negative doubles through...
\newcommand{\negaxthru}{3\,\s}
\setboolean{penax}{true}
\setboolean{respx}{true} %Responsive
\newcommand{\respxthru}{3\,\s}
\setboolean{maxix}{true} %Maximal
\setboolean{suppx}{true} %Support
\newcommand{\suppxthru}{2\,x}
\setboolean{supxx}{true} %Support redouble
\newcommand{\toxstyletext}{very aggr. w/perfect shape\pfix{}}
\newcommand{\doubleothertext}{Rosencrans, snapdragon\pfix{}} %Other special doubles

%Competitive bidding column - NT overcalls
\newcommand{\directlowtext}{15} %range
\newcommand{\directhightext}{18}
\setboolean{syson}{true} %systems on
\newcommand{\ballowtext}{11} %balancing range
\newcommand{\balhightext}{14}
\setboolean{sysba}{true}
\setboolean{dntco}{true} %conventional NT O/c
\newcommand{\directconvtext}{Raptor: 4\,M -- 5\,+m\pfix{}}
\setboolean{unu2l}{true} %Unusual: 2 lowers
\newcommand{\ntothertext}{minors/Quasi \bc\,\rd} %conventional overcalls (sandwich etc)

%Simple overcalls:
\newcommand{\oclowtext}{8} %point range
\newcommand{\ochightext}{16}
\setboolean{oc4cd}{true}
\newcommand{\twooclowtext}{10} %2-lev point range
\newcommand{\twoochightext}{18}
\setboolean{jostr}{true} %Jump overcalls strong
\setboolean{joint}{true}
\setboolean{jowea}{true} %weak
\setboolean{jocon}{true}
\newcommand{\jumpovercalltext}{first two J/O Roman} %Other info about jump overcalls

\setboolean{nsfor}{true} %New suit forcing
\setboolean{nsnfc}{true} %NF Constructive
\setboolean{nsnon}{true} %NF
\setboolean{onstf}{true} %transfer
\setboolean{jrwea}{true}
\setboolean{jrmix}{true}
\setboolean{jrinv}{true}
\newcommand{\overcallcuetext}{limit+}
\setboolean{occus}{true} % cue response support?
\newcommand{\overcallothertext}{Fit J/S} % Other responses

%Defense vs. NT:
\newcommand{\firstcondition}{Str Direct} %''vs.'' line, first half
\newcommand{\firstvsdbl}{Penalty\pfix{}} %1NT-X
\newcommand{\firstvstwocl}{\rd\ or M + m} %1NT-2C
\newcommand{\firstvstwodi}{Majors\pfix{}} %1NT-2D
\newcommand{\firstvstwohe}{\rh\xfix{}} %1NT-2H
\newcommand{\firstvstwosp}{\s\xfix{}} %1NT-2S
\newcommand{\firstvstwont}{Minors} %1NT-2NT
\newcommand{\secondcondition}{Weak or Bal} %the same six lines, right half
\newcommand{\secondvsdbl}{Penalty\pfix{}}
\newcommand{\secondvstwocl}{Nat}
\newcommand{\secondvstwodi}{Nat}
\newcommand{\secondvstwohe}{Nat}
\newcommand{\secondvstwosp}{Natural}
\newcommand{\secondvstwont}{Natural}
\newcommand{\othernt}{3x NAT PRE} %two lines of other NT defense info

%Direct cuebid
\setboolean{qamim}{true} %Michaels
\setboolean{qqmim}{true}
\setboolean{qnmim}{true}
\setboolean{qnmam}{true}
\setboolean{qamin}{true} %Natural
\setboolean{qqmin}{true}
\setboolean{qnmin}{true}
\setboolean{qnman}{true}
\setboolean{qamio}{true} %Other
\setboolean{qqmio}{true}
\setboolean{qnmio}{true}
\setboolean{qnmao}{true}
\newcommand{\cuebiddescribe}{Mathé vs Str, NAT vs Polish\pfix{}} %"Describe" other cuebid comments

%Over opp's takeout double
\setboolean{nsf2l}{true} %New suit forcing?
\setboolean{xnstf}{true} %New suit transfer?
\newcommand{\toxtrftext}{except 1NT\pfix{}} %transfers through?
\setboolean{jswea}{true} %Jump shifts
\setboolean{jsinv}{true}
\setboolean{jsfor}{true}
\setboolean{jsfit}{true}
\setboolean{xximp}{true} %Rdbl 10+
\setboolean{xxcon}{true}
\newcommand{\xxtext}{10+, defence} %Blank space next to Rdbl implies no fit. Can write in ``any 10+''
\setboolean{minnt}{true} % 1m-X-2NT Nat
\setboolean{minrs}{true}
\newcommand{\xtwontminormin}{6}
\newcommand{\xtwontminormax}{9}
\setboolean{majnt}{true}
\setboolean{majrs}{true}
\newcommand{\xtwontmajormin}{10}
\newcommand{\xtwontmajormax}{+}
\newcommand{\toxothertext}{1\,m--X--3\,m LR} %Line at bottom of Opps TOX section

% Opening preempts
\newcommand{\threelevelstyletop}{Sound V1\textsuperscript{st}, any 2\textsuperscript{nd}}
\newcommand{\threelevelstylebottom}{Aggressive NV1\textsuperscript{st}, any 3\textsuperscript{rd}}
\newcommand{\threelevelresp}{NSF}
\newcommand{\fourlevelstyle}{usually 8\pfix{}}
\newcommand{\fourlevelresp}{mumble}
\setboolean{namya}{true}
\newcommand{\fourlevelother}{SOL or 1-loser}

%vs. opening preempts
\newcommand{\optwontoc}{15--17 Bal w/stop\pfix{}} % (2x)-2NT
\setboolean{opdto}{true}
\newcommand{\opdtothru}{4\rh} %Takeout through...
\setboolean{oppen}{true}
\setboolean{opleb}{true} %Lebensohl
\newcommand{\oplebtext}{slow shows} %leb text
\newcommand{\opcuebid}{stopper ask\pfix{}}
\newcommand{\opjumpoc}{Leaping Michaels\pfix{}}
\newcommand{\opothertext}{spl only in pre suit\pfix{}} %Other line

%Slam conventions
\setboolean{gednt}{true} %Gerber
\setboolean{geseq}{true} %NT Seq G
\setboolean{genon}{true} %non-seq G
\newcommand{\gerbertext}{1430/Sty or Trf\pfix{}}
\setboolean{black}{true} %Blackwood
\setboolean{b0314}{true} %RKC
\setboolean{b1430}{true} %1430
\newcommand{\blackwoodtext}{0123 1\textsuperscript{st} round}
\newcommand{\controlcuetext}{1\textsuperscript{st}/2\textsuperscript{nd}, Frivolous 3NT}
\newcommand{\slaminterftext}{DFS/PSS, DEPO (5 trump+)\pfix{}}
\newcommand{\slamothertext}{2\c\ opener's suit is trump unless suit agreement\pfix{}} %Two lines for slam convention info

%Carding:
\setboolean{stasu}{true} %std attitude vs suits
\setboolean{stcsu}{true} %count
\setboolean{udasu}{true} %upside down attitude suits
\setboolean{udcsu}{true} %count
\setboolean{stant}{true} %std attitude vs NT
\setboolean{stcnt}{true} %count
\setboolean{udant}{true} %upside down attitude NT
\setboolean{udcnt}{true} %count
\newcommand{\cardexcepttext}{Alarm clock honours}
\setboolean{smisu}{true} %Smith echo - suit
\setboolean{smint}{true} %Smith - NT
\setboolean{smirv}{true} %reverse Smith
\newcommand{\othercardingtext}{Foster Echo}
\newcommand{\trumpsignaltext}{suit pref w/obvious\pfix{}}

% Signals:
\setboolean{deatt}{true} %Primary signal to declarer's lead
\setboolean{decou}{true}
\setboolean{desui}{true}
\setboolean{pratt}{true} %Primary signal to partner's lead
\setboolean{prcou}{true}
\setboolean{prsui}{true}
\newcommand{\signalexcepttext}{Obvious Shift Trick 1}
\newcommand{\signaltexttop}{lavinthal vs NT only\pfix{}}
\newcommand{\signaltextbottom}{mutterings\pfix{}}

\setboolean{std1d}{true}
\setboolean{udn1d}{true}
\setboolean{lav1d}{true}
\setboolean{odd1d}{true}
\setboolean{oth1d}{true}

%Leads:
%Leads vs. suit:  indicate whether to circle the 1st, 2nd, etc. card (more than one, or zero, is OK)
%e.g. if you lead Ace from Ace-King, \newcommand{AKx}{1}
%e.g. if you lead the default from Queen-Jack, \newcommand{QJx}{}
\setboolean{su4th}{true} %4th best vs. suit
\setboolean{su3rd}{true} %3/5 suit
\setboolean{su3lo}{true} %3/low suit
\setboolean{suatt}{true} %attitude suit
\setboolean{susxx}{true} %low from xx
\newcommand{\suitxx}{1}
\newcommand{\suitxxx}{2}
\newcommand{\suitxxxx}{4}
\newcommand{\suitxxxxx}{4}
\newcommand{\suitHxx}{3}
\newcommand{\suitHxxx}{4}
\newcommand{\suitHxxxx}{4}
\newcommand{\suitafterfirst}{K from AK}
\newcommand{\AKx}{1}
\setboolean{suakv}{true}
\newcommand{\suitAKxvaries}{K Kount}
\newcommand{\KQx}{1}
\newcommand{\QJx}{2}
\newcommand{\JTx}{3}
\newcommand{\TNx}{1}
\newcommand{\KJTx}{2}
\newcommand{\KTNx}{4}
\newcommand{\suitQTNx}{3}
\newcommand{\suitexcepttop}{mutter}
\newcommand{\suitexceptbottom}{frequent suit preference leads\pfix{}}

%Leads vs. NT:
\setboolean{nt4th}{true} %4th best vs. NT
\setboolean{nt3rd}{true} %3/5 NT
\setboolean{nt3lo}{true}
\setboolean{ntatt}{true} %Attitude vs NT
\setboolean{nt2xx}{true} %2nd from xxxx+
\newcommand{\ntxx}{12}
\newcommand{\ntxxx}{123}
\newcommand{\ntxxxx}{1234}
\newcommand{\ntxxxxx}{12345}
\newcommand{\ntHxx}{123}
\newcommand{\ntHxxx}{1234}
\newcommand{\ntHxxxx}{12344}
\newcommand{\ntafterfirst}{K from AK}
\newcommand{\AKxx}{1234}
\setboolean{ntakv}{true}
\newcommand{\ntAKxvaries}{K Kount/unblock}
\newcommand{\KQJx}{1234}
\newcommand{\ntKQTN}{1234}
\newcommand{\QJTx}{1234}
\newcommand{\JTNx}{1234}
\newcommand{\AQJx}{1234}
\newcommand{\AJTx}{1234}
\newcommand{\ntKTNx}{1234}
\newcommand{\ntQTNx}{1234}
\newcommand{\ntexcepttop}{mutter}
\newcommand{\ntexceptbottom}{frequent underlead aces\pfix{}}

% END of pre-set blanks on the convention card

% Finally, if you want to insert a completely new field in the place of your
% choice, you may do so by placing additional TikZ commands into this wrapper
% if you don't want to edit the style file.

% If not, leave it blank as it is below.

% To create a new text field:
% \node at (x,y) {Text to position at this location};
% The semicolon is essential!

\newcommand{\additionaltikz}{%
%
}%end of \additionaltikz
\drawconventioncard
\end{document}
